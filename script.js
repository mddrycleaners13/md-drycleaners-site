...
// Render services: group by category, grid inside
...
(function(){ var d = new Date(); var yyyy = d.getFullYear(), mm = String(d.getMonth()+1).padStart(2,'0'), dd = String(d.getDate()).padStart(2,'0'); if(q('#date')){ q('#date').value = yyyy+'-'+mm+'-'+dd; q('#date').min = yyyy+'-'+mm+'-'+dd; populateSlots(q('#date').value); q('#date').addEventListener('change', function(){ populateSlots(q('#date').value); }); } })();

// Clear
if(q('#clearBtn')) q('#clearBtn').addEventListener('click', function(){ document.querySelectorAll('input[type=number]').forEach(function(i){i.value=0}); cart={}; renderSummary(); if(q('#pickupForm')){ q('#pickupForm').reset(); var d=new Date(); var yyyy=d.getFullYear(), mm=String(d.getMonth()+1).padStart(2,'0'), dd=String(d.getDate()).padStart(2,'0'); q('#date').value = yyyy+'-'+mm+'-'+dd; populateSlots(q('#date').value); } });

// Pickup submit
if(q('#pickupForm')) q('#pickupForm').addEventListener('submit', async function(e){ e.preventDefault(); if(Object.keys(cart).length===0){ alert('Please select at least one service'); return; } var name = q('#name').value.trim(), phone = q('#phone').value.trim(), address = q('#address').value.trim(); var date = q('#date').value, slot = q('#slot').value, notes = q('#notes').value.trim(); if(!name||!phone||!address||!date||!slot){ alert('Please fill all required fields'); return; } var items = Object.keys(cart).map(function(k){ var i = cart[k]; return {id:i.id, name:i.name, qty:i.qty, subtotal:i.subtotal}; }); var total = items.reduce(function(s,i){return s+i.subtotal},0); q('#loading').style.display='flex'; q('#submitBtn').disabled=true; q('#clearBtn').disabled=true; try{ await fetch(GAS_ENDPOINT, { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ formType:'pickup', name:name, phone:phone, address:address, items:items, pickupDate:date, pickupTime:slot, total:total, notes:notes }) }); }catch(err){ console.warn('GAS save failed', err); } q('#loading').style.display='none'; q('#submitBtn').disabled=false; q('#clearBtn').disabled=false; showConfirm(); showToast('✅ Pickup request saved'); document.querySelectorAll('input[type=number]').forEach(function(i){ i.value=0 }); cart={}; renderSummary(); if(q('#pickupForm')){ q('#pickupForm').reset(); var d=new Date(); var yyyy=d.getFullYear(), mm=String(d.getMonth()+1).padStart(2,'0'), dd=String(d.getDate()).padStart(2,'0'); q('#date').value = yyyy+'-'+mm+'-'+dd; populateSlots(q('#date').value); } });

// Contact popup
if(q('#contactPopupBtn')) q('#contactPopupBtn').addEventListener('click', function(){ var p = q('#contactPopup'); p.style.display='flex'; p.setAttribute('aria-hidden','false'); });
if(q('#contactCancel')) q('#contactCancel').addEventListener('click', function(){ var p = q('#contactPopup'); p.style.display='none'; p.setAttribute('aria-hidden','true'); });

if(q('#contactForm')) q('#contactForm').addEventListener('submit', async function(e){ e.preventDefault(); var name = q('#c_name').value.trim(), phone = q('#c_phone').value.trim(), msg = q('#c_message').value.trim(); if(!name||!phone||!msg){ alert('Please fill all fields'); return; } q('#loading').style.display='flex'; try{ await fetch(GAS_ENDPOINT, { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ formType:'contact', name:name, phone:phone, message:msg }) }); }catch(err){ console.warn('Contact save failed', err); } q('#loading').style.display='none'; var p = q('#contactPopup'); p.style.display='none'; p.setAttribute('aria-hidden','true'); showConfirm(); showToast('✅ Message sent'); q('#contactForm').reset(); }});

// Confirm popup and toast
function showConfirm(){ var c = q('#confirmPopup'); c.style.display='flex'; c.setAttribute('aria-hidden','false'); }
if(q('#closePopup')) q('#closePopup').addEventListener('click', function(){ var c = q('#confirmPopup'); c.style.display='none'; c.setAttribute('aria-hidden','true'); });

function showToast(msg){ var t = q('#toast'); t.textContent = msg; t.style.display = 'block'; setTimeout(function(){ t.style.display = 'none'; }, 3500); }

// Utilities
if(q('#callBtn')) q('#callBtn').addEventListener('click', function(){ if(confirm('Call 9821266799? Press OK for 9821266799, Cancel for 9811993799')){ location.href='tel:+919821266799'; } else { location.href='tel:+919811993799'; } });
if(q('#waBtn')) q('#waBtn').addEventListener('click', function(){ var text = encodeURIComponent("Hi! I'd like to schedule a pickup from M.D. Dry Cleaners."); window.open('https://wa.me/919821266799?text='+text,'_blank'); });

// Scroll buttons
if(q('#pickupBtn')) q('#pickupBtn').addEventListener('click', function(){ q('#pickup').scrollIntoView({behavior:'smooth'}); });
if(q('#contactScroll')) q('#contactScroll').addEventListener('click', function(){ q('#contactUs').scrollIntoView({behavior:'smooth'}); });

// Share
if(q('#shareBtn')) q('#shareBtn').addEventListener('click', async function(){ var url = window.location.href; if(navigator.share){ try{ await navigator.share({ title:'M.D. Dry Cleaners', text:'Book a pickup', url: url }); return; }catch(e){} } try{ await navigator.clipboard.writeText(url); alert('Link copied'); }catch(e){ prompt('Copy link', url); } });
