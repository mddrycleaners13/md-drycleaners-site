...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // Render services into multi-column section per category
  const container = q('#serviceGrids');
  container.innerHTML = '';
  for(const cat in SERVICES){
    const section = create('div','service-section');
    const h = create('h4'); h.textContent = cat; section.appendChild(h);
  ...
  // pickup submit
  if(q('#pickupForm')) q('#pickupForm').addEventListener('submit', async (e)=>{ e.preventDefault(); if(Object.values(cart).length===0){ alert('Please select at least one service'); return; } const name = q('#name').value.trim(), phone = q('#phone').value.trim(), address=q('#address').value.trim(); const date=q('#date').value, slot=q('#slot').value, notes=q('#notes').value.trim(); if(!name||!phone||!address||!date||!slot){ alert('Please fill all required fields'); return; } const items = Object.values(cart).map(i=>({id:i.id,name:i.name,qty:i.qty,subtotal:i.subtotal})); const total = items.reduce((s,i)=>s+i.subtotal,0); q('#loading').style.display='flex'; q('#submitBtn').disabled=true; q('#clearBtn').disabled=true; try{ const res = await fetch(GAS_ENDPOINT, {method:'POST',headers:{'Content-Type':'application/json'},body: JSON.stringify({formType:'pickup', name, phone, address, items, pickupDate:date, pickupTime:slot, total, notes})}); const text = await res.text(); console.log('✅ Pickup request: server response ->', text); }catch(err){ console.error('❌ Failed to send pickup to GAS', err); } q('#loading').style.display='none'; q('#submitBtn').disabled=false; q('#clearBtn').disabled=false; showConfirm(); showToast('✅ Pickup request saved'); document.querySelectorAll('input[type=number]').forEach(i=>i.value=0); cart={}; renderSummary(); if(q('#pickupForm')){ q('#pickupForm').reset(); const d=new Date(); const yyyy=d.getFullYear(), mm=String(d.getMonth()+1).padStart(2,'0'), dd=String(d.getDate()).padStart(2,'0'); q('#date').value=yyyy+'-'+mm+'-'+dd; populateSlots(q('#date').value); } });
  ...
  if(q('#contactForm')) q('#contactForm').addEventListener('submit', async (e)=>{ e.preventDefault(); const name=q('#c_name').value.trim(), phone=q('#c_phone').value.trim(), msg=q('#c_message').value.trim(); if(!name||!phone||!msg){ alert('Please fill all fields'); return; } q('#loading').style.display='flex'; try{ const res = await fetch(GAS_ENDPOINT, {method:'POST',headers:{'Content-Type':'application/json'},body: JSON.stringify({formType:'contact', name, phone, message:msg})}); const text = await res.text(); console.log('✅ Contact message: server response ->', text); }catch(err){ console.error('❌ Failed to send contact to GAS', err); } q('#loading').style.display='none'; const p=q('#contactPopup'); p.style.display='none'; p.setAttribute('aria-hidden','true'); showConfirm(); showToast('✅ Message sent'); q('#contactForm').reset(); });
